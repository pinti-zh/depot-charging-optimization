...
def run_main(
    ...
    if simulation_algorithm == "greedy":
        ...
        simulator = PeakShavingSimulator(
            data_input, max_charging_power * data_input.max_charging_power
        )
    else:
...
@click.option(
    ...
    "-epf",
    type=str,
    default="data/energy_price.csv",
    help="energy price file",
)
@click.option(
    "--ce_function",
    "-cef",
...
@click.option(
    ...
    type=float,
    default=1.0,
    help="constant for charging efficiency function",
)
...
def main(
    ...
    return run_main(
        ce_function, simulation_algorithm, alpha, debug, max_charging_power, file_config
    )
