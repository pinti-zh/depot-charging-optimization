...
def charge_on_arrival(env: Environment) -> list[float]:
    ...
    for charge_possible, soe, cap in zip(
        env.state.in_depot, env.state.state_of_energy, env.state.battery_capacity
    ):
        if charge_possible:
            ...
            if effective_charging_power > env.charger.max_possible_effective_charging_power():
                ...
                policy.append(
                    env.charger.inverse_effective_charging_power(effective_charging_power)
                )
        else:
            policy.append(0.0)
    return policy
...
def peak_shaving(env: Environment, max_power_ratio: float = 1.0, *args, **kwargs) -> list[float]:
    ...
    for i, (charge_possible, soe, cap) in enumerate(
        ...
        if charge_possible:
            ...
            try:
                power_required = env.charger.inverse_effective_charging_power(
