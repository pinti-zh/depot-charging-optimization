...
class CasadiOptimizer:
    ...
    def solve(self) -> Solution | None:
        ...
        # Solve with bounds
        sol = solver(
            x0=ca.DM.zeros(self._x.size1(), 1),
            lbx=self._lbx,
            ubx=self._ubx,
            lbg=self._lbg,
            ubg=self._ubg,
        ...
        energy_cost = ca.sum1(
            var_values["total_charging_power"]
            * time_delta.T
            * self._energy_price
            / factor
        )
        ...
        power_cost = (
            var_values["max_charging_power"].item() * self._grid_tariff / factor
        )
        ...
        return Solution(
            ...
            max_charging_power_used=var_values["max_charging_power"].item()
            / self._factor_cp,
            charging_power=[
                cp / self._factor_cp for cp in var_values["charging_power"]
            ],
            effective_charging_power=[
                cp / self._factor_cp for cp in var_values["effective_charging_power"]
            ],
            state_of_energy=[
                soe / self._factor_soe for soe in var_values["state_of_energy"]
            ],
            lower_soe_envelope=[
                soe / self._factor_soe for soe in var_values["lower_soe_envelope"]
            ],
        )
    ...
    def _set_variables(self) -> None:
        ...
        depot_charge_mask: np.ndarray = np.array(
            self._input_data.depot_charge, dtype=bool
        )
        ...
        self._variables["charging_power"]["lb"] = -(
            bidirectional_charging_mask * depot_charge_mask
        ).astype(float)
        ...
        self._variables["effective_charging_power"]["lb"] = (
            -2 * bidirectional_charging_mask * depot_charge_mask
        ).astype(float)
        self._variables["effective_charging_power"]["ub"] = depot_charge_mask.astype(
            float
        )
        ...
        self._variables["max_charging_power"]["var"] = ca.SX.sym(
            "max_charging_power", 1
        )
        self._variables["max_charging_power"]["lb"] = np.zeros((1,), dtype=float)
        self._variables["max_charging_power"]["ub"] = (
            self._input_data.num_vehicles * np.ones((1,), dtype=float)
        )
        ...
        if self._config.initial_soe is not None:
            self._initial_soe = ca.DM(
                np.array(self._config.initial_soe, dtype=float) * self._factor_soe
            )

        self._energy_demand = ca.DM(
                ...
                [
                    np.array(
                        [
                            t2 - t1
                            for t1, t2 in zip(
                                [0] + self._input_data.time[:-1], self._input_data.time
                            )
                        ],
                        dtype=float,
                    )
    ...
    def _set_constraints(self) -> None:
        # assert all optimization variables are initialized
        assert isinstance(
            self._variables["charging_power"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._variables["effective_charging_power"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._variables["total_charging_power"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._variables["max_charging_power"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._variables["state_of_energy"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._variables["lower_soe_envelope"]["var"], ca.SX
        ), "uninitialized optimization variable"
        assert isinstance(
            self._energy_demand, ca.DM
        ), "uninitialized optimization variable"
        assert isinstance(
            self._realistic_worst_case, ca.DM
        ), "uninitialized optimization variable"
        assert isinstance(
            self._time_delta, ca.DM
        ), "uninitialized optimization variable"
        ...
        self._constraints["max_charging_power_relaxation"] = {
            "expr": self._variables["total_charging_power"]["var"]
            - self._variables["max_charging_power"]["var"],
            "lb": -float("inf") * np.ones(self._input_data.num_timesteps, dtype=float),
            "ub": np.zeros(self._input_data.num_timesteps, dtype=float),
        }
        ...
        self._constraints["energy_flow"] = {
            ...
            - self._variables["effective_charging_power"]["var"]
            * self._time_delta
            * power_to_soe
            + self._energy_demand,
            "lb": np.zeros(
            ...
            for i in range(self._input_data.num_vehicles):
                if not np.isnan(self._initial_soe[i]):
                    self._constraints[f"initial_soe_{i}"] = {
                        "expr": self._variables["state_of_energy"]["var"][i, 0]
                        - self._initial_soe[i],
